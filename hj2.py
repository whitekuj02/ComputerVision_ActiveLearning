...
wandb.init(
    ...
    name = "SGD + 노멀라이즈 + early_stop(희진) + 증강(RandomHorizontalFlip,RandomAffine) + Pre_trained + centerCrop(448) + softloss", # 매 실험마다 이름 바꾸어주기
...
aug_train1 = transforms.Compose([
    ...
    transforms.RandomAffine(0, translate=(shift_range/448, shift_range/448)),
    transforms.CenterCrop((448, 448)),
    transforms.ToTensor(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
])
...
train_set = ConcatDataset([train_set, aug_set, aug_set1])
